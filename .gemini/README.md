...
Esta pasta contém 4 pilares fundamentais de documentação:
...
3. **Tipagem Estrita Obrigatória**: Nunca use `any`. Todo payload externo (API, formulários, localStorage) deve possuir interfaces e tipagens estritas.
4. **Consistência Visual**: Respeite os tokens do Design System definidos em [`visual-identity.md`](./visual-identity.md) e `src/styles.css`. Fundo preto profundo (`--background: #09090b`), texto branco e acentos em roxo neon (`--primary: #9333ea`).
5. **Segurança por Padrão**: Toda interação com banco de dados via Supabase deve respeitar e exigir políticas de Row Level Security (RLS). Nenhuma credencial ou chave sensível de backend deve ser exposta no client.
