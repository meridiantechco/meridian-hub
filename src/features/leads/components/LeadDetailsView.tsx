...
import type { InteracaoItem, LeadItem } from "../types";
...
export function LeadDetailsView({ leadId }: LeadDetailsViewProps) {
  ...
  const carregarDados = useCallback(async () => {
    ...
    const [leadObtido, listaInteracoes] = await Promise.all([
      leadsService.obterLeadPorId(leadId),
      leadsService.listarInteracoes(leadId),
    ]);
  ...
  const mudarStatus = async (novoStatus: LeadItem["status"]) => {
    if (!lead) return;
    await leadsService.atualizarStatusLead(lead.id, novoStatus);
    ...
    >
      <div className="space-y-6 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* COLUNA DIREITA: OBSERVAÇÕES & HISTÓRICO */}
            ...
            <Card className="bg-card border-border shadow-elev">
              ...
              <CardContent>
                <Textarea
                  ...
                  placeholder="Ex: Proprietário prefere contato após às 15h..."
...
function ModalEditarRedesLead({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const atualizado = await leadsService.atualizarLead(lead.id, {
        instagram: instaLimpo,
        facebook: faceLimpo,
