...
export const assistantService = {
  async processarPergunta(pergunta: string): Promise<MensagemChat> {
    ...
    if (termo.includes("melhor") || termo.includes("score acima") || termo.includes("top")) {
      ...
      resposta =
        `Encontrei **${topLeads.length} oportunidades de alto score** (Score &ge; 75) na sua base comercial:\n\n` +
        topLeads
          .map(
            (l, i) =>
              `${i + 1}. **${l.nome}** (${l.categoria}) — **Score ${l.score}** · ${!l.tem_site ? "⚡ Sem site" : "🌐 Com site"}`,
          )
          .join("\n") +
      ...
      const emRisco = oportunidades
        .filter(
          (o) =>
            o.categoriaOportunidade.includes("em_risco") ||
            o.categoriaOportunidade.includes("paradas"),
        )
        .slice(0, 5);
      metadadosLeads = emRisco.map((o) => ({
        id: o.lead.id,
        nome: o.lead.nome,
        categoria: o.lead.categoria,
        score: o.score,
      }));

      if (emRisco.length > 0) {
        resposta =
          `Identifiquei **${emRisco.length} contas em risco ou sem contato recente** (&ge; 3 dias sem resposta):\n\n` +
          emRisco
            .map(
              (o, i) =>
                `${i + 1}. **${o.lead.nome}** — ${o.diasSemContato} dias sem contato (${o.lead.status.toUpperCase()})`,
            )
            .join("\n") +
      ...
      termo.includes("precisam de contato") ||
      termo.includes("novo") ||
      termo.includes("sem contato")
    ) {
      const novos = leads.filter((l) => l.status === "novo").slice(0, 5);
      metadadosLeads = novos.map((l) => ({
        id: l.id,
        nome: l.nome,
        categoria: l.categoria,
        score: l.score,
      ...
      resposta =
        `Existem **${novos.length} novos leads** recém-minerados aguardando primeira abordagem comercial:\n\n` +
        novos
          .map(
            (l, i) =>
              `${i + 1}. **${l.nome}** (${l.categoria}) — Score ${l.score} · Tel: ${l.telefone || "Não informado"}`,
          )
          .join("\n") +
        `\n\n🚀 *Dica:* Utilize os scripts de Primeiro Contato no módulo de Templates para acelerar a qualificação.`;
    } else if (
      termo.includes("vendedor") ||
      termo.includes("equipe") ||
      termo.includes("produtividade")
    ) {
      resposta =
      ...
      resposta =
        `Com base nos **${leads.length} estabelecimentos** cadastrados no Meridian Hub:\n\n` +
